...
//
// A senha é definida obrigatoriamente na variável de ambiente ADMIN_PASSWORD.
// Configure essa variável nas configurações de ambiente (Vercel → Settings → Environment Variables).

const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || process.env.NEXT_PUBLIC_ADMIN_PASSWORD || "";

if (!ADMIN_PASSWORD) {
  throw new Error("ADMIN_PASSWORD environment variable is not defined. Configure it in your hosting provider.");
}
